...
export default function SideNavbar() {
  const [buttonPos, setButtonPos] = useState("translateY(-60px)");
  const [translate, setTranslate] = useState("translate(-240px)");

  const cont = useFoldercontext();
  const {newfolder,setNewfolder} = cont;

  const handleSideNav = () => {
    if (translate === "translate(-240px)") {
      setTranslate("translate(0px)");
      setButtonPos("translateX(240px) translateY(-60px)");
    } else {
      setTranslate("translate(-240px)");
      setButtonPos("translateY(-60px)");
    }
  };
  ...
  return (
    <>
      <nav
        id="sidenav-8"
        className="absolute left-0 top-0 z-[1035] h-full w-60 -translate-x-full bg-white shadow-[0_4px_12px_0_rgba(0,0,0,0.07),_0_2px_4px_rgba(0,0,0,0.05)] data-[te-sidenav-hidden='false']:translate-x-0 dark:bg-zinc-800"
      ...
      >
        ...
        >
          <Image
            ...
            draggable="false"
          />
          <span style={{ fontSize: "25px" }}>EXTRACT</span>
        </Link>
        <ul
        ...
        >
          ...
          <li className="relative pt-4">
            ...
            <Link
              href={{ pathname: "/Dashboard", query: { toShow: "folders" } }}
              className="flex cursor-pointer items-center truncate rounded-[5px] px-6 py-[0.45rem] text-[0.85rem] text-gray-600 outline-none transition duration-300 ease-linear hover:bg-slate-50 hover:text-inherit hover:outline-none focus:bg-slate-50 focus:text-inherit focus:outline-none active:bg-slate-50 active:text-inherit active:outline-none data-[te-sidenav-state-active]:text-inherit data-[te-sidenav-state-focus]:outline-none motion-reduce:transition-none dark:text-gray-300 dark:hover:bg-white/10 dark:focus:bg-white/10 dark:active:bg-white/10"
              data-te-sidenav-link-ref
            >
              <span className="mr-4 [&>svg]:h-3.5 [&>svg]:w-3.5 [&>svg]:text-gray-400 dark:[&>svg]:text-gray-300">
...
<!-- Toggler --> */}
      <button
        className="mx-1 mt-1 inline-block rounded bg-primary px-6 py-2.5 text-xs font-medium uppercase leading-tight text-white shadow-md transition duration-150 ease-in-out hover:bg-primary-700 hover:shadow-lg focus:bg-primary-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-primary-800 active:shadow-lg"
        data-te-sidenav-toggle-ref
        data-te-target="#sidenav-8"
        aria-controls="#sidenav-8"
        aria-haspopup="true"
        // style={{marginLeft: "240px", position: "fixed"}}
        style={{
          position: "fixed",
          transition: "0.5s",
          transform: buttonPos,
